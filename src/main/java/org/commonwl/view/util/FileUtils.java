 ...
 *
 * <p>Uses other utilities, such as Apache Commons IO's {@code FileUtils}, but with refinements
 * specific for CWL Viewer (e.g. handling Git repositories).
 *
 * @since 1.4.6
 */
public class FileUtils {
  ...
  /**
   * Deletes the directory of a git repository. Note that the <code>Git</code> object contains a
   ...
   * virtual file system, with a root directory (normally "/"), but that's an abstract
   * representation of the bundle files.
   *
   * <p>This method looks inside the bundle for the bundle source directory. This directory contains
   * a hash in its name structure. Apache Taverna creates temporary directories in
   *
   * <pre>java.io.tmpdir</pre>
   *
   * using this same hash. This method will delete any directory matching that hash in the temporary
   * directory.
   *
   * <p>NOTE: the directory deleted is expected to be empty. It is a left over by Apache Taverna,
   * and failing to delete it shouldn't be a major issue (other than using an inode to represent the
   * file in the file system). And also note that we are not deleting the parent directory, since
   * the left-over directory is always stored in the system temporary directory, while the ROBundle
   * with the hash is stored in the Spring configured directory for bundles.
   *
  ...
  /**
   * Used to delete the directory with a robundle ZIP file. It must be used when the system fails to
   * generate a bundle file. Apache Taverna's API may have already created the Bundle ZIP.
   *
   * <p>Normally this file is stored in a newly created directory (e.g.
   *
   * <pre>/tmp/bundles/bundle-hash-1234/robundle.zip</pre>
   *
   * ), and when we fail to successfully create the bundle, it is best to remove this file to
   * prevent disk space issues (especially if we end up with duplicated bundles).
   *
   * @param bundle A bundle object
   * @throws IOException if it fails to delete the bundle temporary directory
   * @since 1.4.6
